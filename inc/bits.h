...
typedef __uint128_t bitset128_t;

#define U128_BIT_ZERO_INIT(var) \
    do {                        \
        (var) = (__uint128_t)0; \
    } while (0)
...
#define U128_FOR_EACH_BIT_SET(var, pos)            \
    for (__uint128_t _temp = (var);                \
        _temp && (pos = __builtin_ctzg(_temp), 1); \
        _temp &= (_temp - 1))

typedef __uint128_t bitset256_t[2];

/* clang-format off */
#define U256_BIT_ZERO_INIT(var)    \
    do {                           \
        (var)[0] = (__uint128_t)0; \
        (var)[1] = (__uint128_t)0; \
    } while (0)
...
#define U256_FOR_EACH_BIT_SET(var, pos)                                       \
    for (struct { int i; __uint128_t temp; } _s = { 0, (var)[0] };            \
            (_s.temp || (_s.i == 0 && (++_s.i, _s.temp = (var)[1], _s.temp))) \
                && ((pos) = _s.i * 128 + __builtin_ctzg(_s.temp, -1), 1);     \
            _s.temp &= (_s.temp - 1))
/* clang-format on */
